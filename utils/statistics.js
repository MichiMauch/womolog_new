  ...
  export function calculateStatistics(data) {
    ...
    sortedData.forEach(row => {
      ...
      if (!isNaN(startDate) && !isNaN(endDate)) {
        ...
        // Berechnung der Besuche pro Land
        if (!visitsPerCountry[countryCode]) {
          visitsPerCountry[countryCode] = { visitCount: 0, multipleVisitedPlaces: 0, nights: 0 };
        }
        visitsPerCountry[countryCode].visitCount += 1;
  
        // Berechnung der Übernachtungen pro Land
        visitsPerCountry[countryCode].nights += nights;
  
        // Berechnung der Besuche pro Land und Ort
        if (!countryPlaceCount[countryCode]) {
          countryPlaceCount[countryCode] = { uniquePlaces: new Set(), visitCount: 0 };
        }
        countryPlaceCount[countryCode].uniquePlaces.add(place);
        countryPlaceCount[countryCode].visitCount += 1;
  
        // Verfolgung der Besuche pro Ort pro Land
        if (!visitsPerPlacePerCountry[countryCode]) {
          visitsPerPlacePerCountry[countryCode] = {};
        }
        if (!visitsPerPlacePerCountry[countryCode][place]) {
          visitsPerPlacePerCountry[countryCode][place] = 0;
        }
        visitsPerPlacePerCountry[countryCode][place] += 1;
    ...
    Object.entries(visitsPerPlacePerCountry).forEach(([countryCode, places]) => {
      const multipleVisitedPlaces = Object.values(places).filter(count => count > 1).length;
      if (visitsPerCountry[countryCode]) {
        visitsPerCountry[countryCode].multipleVisitedPlaces = multipleVisitedPlaces;
      }
    });
